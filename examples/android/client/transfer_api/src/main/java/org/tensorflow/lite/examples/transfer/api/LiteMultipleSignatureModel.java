...
public class LiteMultipleSignatureModel implements Closeable {
   ...
   * Loads the bottleneck feature from the given image array.
   *
   * @param fileDir 3-D float array of size (IMG_SIZE, IMG_SIZE, 3)
   * @return 1-D float array containing bottleneck features
   */
  ByteBuffer[] loadParameters(Path fileDir) {
    // Load the trained weights from the checkpoint file.
    File outputFile = new File(fileDir.toFile().getAbsolutePath(), "tflite-tl-test.bin");
    Map<String, Object> inputs = new HashMap<>();
    inputs.put("checkpoint_path", outputFile.getAbsolutePath());
    Map<String, Object> outputs = new HashMap<>();
    ByteBuffer[] params = new ByteBuffer[BOTTLENECK_SIZE];
    outputs.put("restore", params);
